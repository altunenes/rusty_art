...
fn cC(uv: vec2<f32>, time: f32, resolution: vec2<f32>) -> vec4<f32> {
    ...
    let mX: f32 = (sin(time * 0.5) + 1.0) * 0.2;
    let mY: f32 = (cos(time * 0.5) + 1.0) * 0.2;
    ...
    for (var i: i32 = 0; i < 45; i++) {
        ...
        let yV: f32 = osc(0.955, 0.999, 10.0, time);
        let zV: f32 = osc(0.7, 0.86, 10.0, time);
...
fn main(@builtin(position) FragCoord: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    var uv: vec2<f32> = 2.0 * (1.5 * FragCoord.xy - resolution.xy) / resolution.y * 1.1;
