...
fn main(@builtin(position) FragCoord: vec4<f32>, @location(0) tex_coords: vec2<f32>) -> @location(0) vec4<f32> {
    ...
    let t: f32 = u_time.time * 0.2;
    let noiseScale: f32 = 3.1;
    var noiseVec: vec2<f32> = vec2<f32>(0.0, 0.0);
    noiseVec.x = fbm(uv + vec2<f32>(t, 0.1)) * noiseScale;
    noiseVec.y = fbm(uv + vec2<f32>(0.2, t)) * noiseScale;
