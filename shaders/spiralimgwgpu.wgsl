...
fn main(@builtin(position) FragCoord: vec4<f32>, @location(0) tex_coords: vec2<f32>) -> @location(0) vec4<f32> {
    ...
    let spiral: f32 = 0.8 * cos(12.0 * (theta + 45.0 * r - 3.9 * u_time.time)) + luminance;
    var col: vec3<f32>;
    col.r = smoothstep(1.0, 0.3, spiral);
    col.g = smoothstep(1.0, 0.3, spiral);
    col.b = smoothstep(1.0, 0.3, spiral);
