...
fn update(app: &App, model: &mut Model, _update: Update) {
    ...
    egui::Window::new("Settings").show(&ctx, |ui| {
        ...
        ui.add(egui::Slider::new(&mut settings.speed, 0.0..=256.0).text("speed"));
...
fn gabor(x: f32, y: f32, kx_ratio: f32, ky_ratio: f32, theta: f32, sigma: f32, width: f32, height: f32, time: f32, speed: f32) -> f32 {
    ...
    (-0.5 * (x_theta.powi(2) / sigma.powi(2) + y_theta.powi(2) / sigma.powi(2))).exp()
        * (2.0 * std::f32::consts::PI * (kx * x_theta + ky * y_theta) + speed * time).cos()
}
fn view(app: &App, model: &Model, frame: Frame) {
    ...
    let theta = settings.t.to_radians(); 
    ...
    for i in 0..width as i32 {
        for j in 0..height as i32 {
            ...
            let value = gabor(x, y, kx_ratio, ky_ratio, theta, sigma, width, height, model.time, speed);
            let color = match settings.c {
                ...
                6 => nannou::color::gray(((value.sin() + 1.0) / 2.0).abs()),
                7 => nannou::color::gray(((model.time % 3.0 + value + 1.0) / 2.0).abs()), 
