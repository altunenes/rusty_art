...
fn update(app: &App, model: &mut Model, update: Update) {
    ...
    egui::Window::new("Shader Settings").show(&ctx, |ui| {
        ui.add(egui::Slider::new(&mut model.settings.lambda, 1.0..=100.0).text("time"));
        ui.add(egui::Slider::new(&mut model.settings.theta, 2.0..=100.0).text("t"));
        ui.add(egui::Slider::new(&mut model.settings.sigma, 0.0..=50.0).text("r"));
...
fn model(app: &App) -> Model {
    ...
    let fs_desc = wgpu::include_wgsl!("../shaders/ornaments.wgsl");
