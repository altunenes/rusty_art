...
fn update(app: &App, model: &mut Model, update: Update) {
    ...
    egui::Window::new("Shader Settings").show(&ctx, |ui| {
        ...
        ui.add(egui::Slider::new(&mut model.settings.lambda, -33.4..=3.4).text("l"));
        ui.add(egui::Slider::new(&mut model.settings.theta, 0.0..=PI).text("t"));
        ui.add(egui::Slider::new(&mut model.settings.alpha, -PI..=PI).text("a"));
        ui.add(egui::Slider::new(&mut model.settings.gamma, -PI..=PI).text("g"));
...
fn model(app: &App) -> Model {
    ...
    let fs_desc = wgpu::include_wgsl!("../shaders/faketunnel.wgsl");
